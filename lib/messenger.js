...
function Messenger( url ) {
	...
	this.publish = function( exchangeName, routingKey, message, messageOptions ) {
		...
		this.getConnection().exchanges[ exchangeName ].publish( routingKey, message, messageOptions );
	}
	...
	this.subscribe = function( queueName, subscribeOptions, messageHandler ) {
		...
		this.getConnection().queues[ queueName ].subscribe( subscribeOptions, messageHandler );
	}
